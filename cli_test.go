...
func TestSkimCmdParse(t *testing.T) {
	action, exitStatus := skimCmdParse([]string{})
	assert.Nil(t, action)
	assert.Equal(t, 1, exitStatus)

	action, exitStatus = skimCmdParse(
		[]string{"t/chain.crt", "t/myserver.crt", "t/myserver-fromca2.crt"})
	assert.NotNil(t, action)
	assert.Equal(t, -1, exitStatus)
}

func TestVerifyChainCmdParse(t *testing.T) {
	action, exitStatus := verifyChainCmdParse([]string{}, []string{}, nil)
	assert.Nil(t, action)
	assert.Equal(t, 1, exitStatus)

	action, exitStatus =
		verifyChainCmdParse([]string{"foo", "fa"}, []string{"bar", "bas"}, []string{"mycert"})
	assert.NotNil(t, action)
	assert.Equal(t, -1, exitStatus)

	action, exitStatus = verifyChainCmdParse(nil, []string{"bar", "bas"}, []string{"mycert"})
	assert.Nil(t, action)
	assert.Equal(t, 1, exitStatus)

	action, exitStatus = verifyChainCmdParse([]string{"foo", "fa"}, []string{"bar", "bas"}, nil)
	assert.Nil(t, action)
	assert.Equal(t, 1, exitStatus)
